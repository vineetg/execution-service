...
db.serialize(function() {
   sql = db.run("CREATE TABLE jobs (command TEXT, status INTEGER, stdout TEXT, stderr TEXT)", function(err) {
      console.log("Skipping creation of jobs table.");
   });
});

exports.addJob = function(command, next) {
   db.run("INSERT INTO jobs VALUES (?, ?, ?, ?)", command, 201, "", "", function(err) {
      if (err)
         return next(err);
...
exports.getJobById = function(id, next) {
   db.get("SELECT command, status, stdout, stderr FROM jobs WHERE rowid = ?", id, function(err, row) {
      if (err)
         return next(err);
...
exports.getJobsByStatus = function(status, next) {
   db.all("SELECT rowid, command FROM jobs WHERE status = ?", status, function(err, rows) {
      if (err) {
